...
import { ouvrirJournal, issue } from "./journal.js";
import { execFileSync } from "node:child_process";
import { loadavg, cpus } from "node:os";
import { dirname } from "node:path";
import { generateRecords, generateAlerts, FIELDS } from "./corpus.js";
import { TIERS, ENCODEURS, GENERATIFS, loadExtractors, loadClassifiers, loadGeneratifs, extract, classify, correct, OLLAMA, estLocal, PROMPTS, rechauffer, residents } from "./tiers.js";
 ...
 */
export const DECOUPAGE_DE_MESURE = "heldout";
...
export function empreinteDuReleve(profils) {
...
export function readProfiles() {
    if (existsSync(FICHIER)) {
        ...
        const attendue = p.empreinte;
        const calculee = empreinteDuReleve(p);
        if (typeof attendue !== "string") {
            throw new Error(`${FICHIER} carries no content fingerprint: there is no way to tell whether its\n`
        ...
        if (attendue !== calculee) {
            throw new Error(`${FICHIER} has changed since it was measured — fingerprint ${attendue}, content ${calculee}.\n`
    ...
    const livres = readdirSync(racine)
        ...
        .map((f) => {
        try {
            return { f, p: JSON.parse(readFileSync(join(racine, f), "utf8")) };
        }
        catch {
            return null;
        }
    ...
    {
        const attendue = ref.p.empreinte;
        const calculee = empreinteDuReleve(ref.p);
        if (typeof attendue !== "string") {
            throw new Error(`${ref.f} carries no content fingerprint: there is no way to tell whether its\n`
                + `  figures are the ones that were measured. Re-seal it: npm run sceller -- ${ref.f}`);
        }
        if (attendue !== calculee) {
            throw new Error(`${ref.f} has changed since it was measured — fingerprint ${attendue}, content ${calculee}.\n`
...
function etatDuDepot() {
    try {
        ...
        const sale = execFileSync("git", ["status", "--porcelain"], { cwd: fileURLToPath(new URL("..", import.meta.url)), encoding: "utf8" }).trim().length > 0;
        return { commit, sale };
    }
    catch {
        return undefined;
...
export async function measure(howMany = 120, options = {}) {
    ...
    const combien = (t) => options.cases?.[t] ?? howMany;
    ...
    const sauver = (ex, cl, lt) => {
        ...
        const partiel = {
            ...
            code: version,
            chargeAvantPasse,
            provenance: { ...(ancien?.provenance ?? {}), ...provenance },
    ...
    profils.empreinte = empreinteDuReleve(profils);
...
if (isMain(import.meta)) {
    ...
    const prompt = brutPrompt;
    const parPalier = Object.fromEntries(GENERATIFS.map((t) => [t, casesGen]));
