...
import { TIERS, loadExtractors, loadClassifiers, extract, classify, correct } from "./tiers.js";
...
export async function measure(howMany = 120) {
    /*
    ...
    for (const tier of TIERS) {
        ...
        for (const champ of FIELDS) {
            ...
            for (const d of dossiers) {
                if (correct(await extract(tier, d, champ), d.truth[champ]))
                    right++;
            }
    ...
    for (const tier of TIERS) {
        ...
        for (const a of alertes)
            if (await classify(tier, a) === a.truth)
                right++;
        const duration = performance.now() - start;
        classification[tier] = {
