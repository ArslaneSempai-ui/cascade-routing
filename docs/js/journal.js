...
import { normaliserReponse, correct } from "./tiers.js";
import { pairedVerdict } from "./interval.js";
 ...
 */
export function issue(got, expected) {
    if (correct(got, expected))
        return "clean";
...
export function elaguer(dossier = DOSSIER, garde = GARDE_DERNIERS) {
    let fichiers;
    try {
...
export function ouvrirJournal(nom, conditions) {
 ...
 */
export function lireJournal(chemin) {
...
const cle = (t) => `${t.field}|${t.caseId}`;
/** McNemar entre deux conditions quelconques, sur les cas qu'elles ont réellement en commun. */
export function apparie(tentatives, a, b) {
    const prendre = (s) => {
        const m = new Map();
        for (const t of tentatives) {
            if (t.tier !== s.tier)
                continue;
            if (s.phrasing !== undefined && t.phrasing !== s.phrasing)
                continue;
            m.set(cle(t), t);
        }
        return m;
 ...
 */
export function parDocument(tentatives, s) {
    const docs = new Map();
    for (const t of tentatives) {
        if (t.tier !== s.tier)
            continue;
    ...
    for (const [id, champs] of docs) {
        if (s.champsRequis !== undefined && champs.length !== s.champsRequis) {
            ecartes++;
            continue;
        }
    ...
    let propres = 0;
    for (const [, champs] of complets)
        if (champs.every((t) => t.outcome === "clean"))
            propres++;
...
export function issues(tentatives, s) {
    const c = { clean: 0, blank: 0, wrong: 0 };
    for (const t of tentatives) {
        if (s?.tier !== undefined && t.tier !== s.tier)
            continue;
        if (s?.phrasing !== undefined && t.phrasing !== s.phrasing)
            continue;
        if (s?.field !== undefined && t.field !== s.field)
            continue;
        c[t.outcome]++;
    }
 ...
 */
export function desaccord(tentatives, a, b) {
    const prendre = (s) => {
        const m = new Map();
        for (const t of tentatives) {
            if (t.tier !== s.tier)
        ...
        if (ta.outcome === "clean" && tb.outcome === "clean") {
            ...
            if (different) {
                ...
                if (exemples.length < 10)
                    exemples.push({ field: ta.field, caseId: ta.caseId, a: ta.value, b: tb.value });
            }
        }
        if (ta.outcome === tb.outcome && different)
            memeIssueValeursDifferentes++;
    }
    return {
 ...
 */
export function latences(lots) {
    const machines = new Set(lots.map((l) => l.conditions?.machine?.cpu ?? "inconnue"));
    if (machines.size > 1) {
        throw new Error(`Refus de grouper des latences venues de ${machines.size} machines : ${[...machines].join(", ")}.\n`
    ...
    const q = (f) => ms.length ? ms[Math.min(ms.length - 1, Math.floor(f * ms.length))] : null;
...
export function accordEntreMachines(a, b) {
    ...
    for (const tb of b) {
        ...
        if (ta.value === tb.value)
            memeChaine++;
        else if (divergences.length < 10)
            divergences.push({ key: `${tb.tier}/${tb.field}/${tb.caseId}`, a: ta.value, b: tb.value });
    }
    return {
 ...
 */
export function annoter(chemin, note) {
