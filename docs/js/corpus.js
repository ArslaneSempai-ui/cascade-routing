 ...
 */
export const FIELDS = ["name", "birth", "document", "country", "address"];
/*
 ...
 */
const TRAINING = [
    (c) => `Client: ${c.name}, born ${c.birth} in ${c.birthCity}, residing at ${c.address}. Identity document ${c.document}, issued in ${c.country}.`,
    (c) => `Onboarding file — name ${c.name} / DOB ${c.birth} / ID ${c.document} / nationality ${c.country}. Correspondence address: ${c.address}.`,
    (c) => `The applicant, ${c.name}, was born on ${c.birth}. They hold document number ${c.document} delivered by the authorities of ${c.country} and live at ${c.address}.`,
    (c) => `${c.name} (${c.country}), d.o.b. ${c.birth}. Address on file: ${c.address}. Document reference ${c.document}.`,
    (c) => `Received today: an application from ${c.name}. Date of birth is ${c.birth} and the identity document presented carries the number ${c.document}. Nationality: ${c.country}. The declared address is ${c.address}.`,
];
/**
 * The test half.
 *
 ...
 */
const HELDOUT = [
    (c) => `${c.name} — dob ${c.birth} — doc no ${c.document} — ${c.country} — lives ${c.address} (updated by branch staff, no further checks)`,
    (c) => `Further to our call: the person concerned is ${c.name}. We hold ${c.document} on file. Born ${c.birth}. Their postal address reads ${c.address} and they present themselves as a national of ${c.country}.`,
