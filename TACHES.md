...
- [x] ~~**Les 6 PRs dependabot**~~ — **cinq fermées le 27–28/08, la sixième reportée**. #1–3 (actions GitHub) fusionnées ; #6 `@types/node` reprise à la main en `a29cf6b` ; #4 `transformers` 4.2.0 reprise en `95d4643`. Aucune n'a fait bouger un taux publié : les montées ne déplacent que les comptes d'inventaire, et `POIDS_MODELES` garde ses révisions, ses octets et sa disposition — mesuré, pas supposé, par empreinte du cache avant/après un vrai chargement. La garde d'inventaire n'a jamais été contournée : c'est elle qui rend ces montées manuelles, et elle a eu raison à chaque fois

- [ ] **`typescript` 7.0.2 : reporté après la refonte du design** (décision d'Arslane, 28/08). Ce n'est pas un blocage technique — la suite passe et le paquet web compilé est **identique octet pour octet** entre 5.9.3 et 7.0.2, vérifié en construisant les deux côte à côte. Deux choses à savoir le jour où on la reprend : TS 7 est le portage natif, il ajoute **20 paquets binaires de plateforme** au verrou (ils se replient en une famille `@typescript/typescript`, donc l'inventaire concorde toujours entre macOS et le coureur Linux) ; et il refuse `tsconfig.web.json` en `TS5011` faute de `rootDir` explicite. Le correctif est d'une ligne, `"rootDir": "./src"` à côté de `"outDir"`, et il resservira tel quel
