...
const pc = (r: Rate | undefined, gras = false) =>
  !r ? "n/a" : gras ? `**${(r.rate * 100).toFixed(1)} %**` : `${(r.rate * 100).toFixed(1)} %`;
...
export function sonde(p: Profiles): string {
  ...
  w(table(
    ["Field", ...PALIERS.map((t) => `\`${NOM[t]}\``)],
    FIELDS.map((f) => [`\`${f}\``, ...PALIERS.map((t) => { const r = ex(t, f); return r ? writeRate(r) : "n/a"; })]),
  ));
  w(``);
  ...
  w(table(
    PALIERS.map((t) => `\`${NOM[t]}\``),
    [PALIERS.map((t) => { const r = cl(t); return r ? writeRate(r) : "n/a"; })],
  ));
  w(``);
  w(`## Latency, per call`);
  w(``);
  w(table(
    PALIERS.map((t) => `\`${NOM[t]}\``),
    [PALIERS.map((t) => { const l = lat(t); return l === undefined ? "n/a" : l < 1 ? "<1 ms" : `${Math.round(l).toLocaleString("en-US")} ms`; })],
  ));
  w(``);
