...
| 2026-08-20 | Les latences publiées sont celles des modèles, mesurées un élément à la fois sur machine au repos. | Elles étaient celles des modèles **plus un pilote audio que personne n'avait regardé**. `UA Mixer Engine` occupait un cœur sur dix depuis seize jours ; l'arrêter a retiré 22 à 36 % de la latence de chaque palier, et fait passer la chaîne retenue de 1 341 à 968 ms. Les exactitudes sont restées identiques sur les 35 combinaisons palier×champ : rien du modèle n'avait changé. | Une passe contaminée par mon propre travail concurrent, qui a rendu `gen-8b` 32 % plus lent et m'a fait relever la charge machine pour la première fois. |
| 2026-08-20 | Everything runs locally. No API key, nothing leaves the machine. | Vrai par défaut, faux sur une configuration que rien n'interdisait. Le chemin d'une mesure ne contient qu'un seul appel sortant — l'hôte génératif — et `OLLAMA_HOST` est une variable d'environnement : pointée sur un serveur d'équipe, chaque document part chez ce tiers. La promesse était conditionnelle et sa condition n'était écrite nulle part. | Le recensement de tout ce qui peut ouvrir une connexion dans `src/`, fait pour transformer l'affirmation en preuve. |
| 2026-08-20 | Deux fois de suite, une explication plausible de la même observation : d'abord que les paliers génératifs étaient instables d'une passe à l'autre, ensuite que leur latence suivait la charge de la machine. | Les deux étaient fausses. Les paliers sont déterministes — 35 combinaisons palier×champ identiques au millième sur quatre passes. Et la charge n'explique pas l'écart : mesuré exprès sous une charge de 8,35, `gen-0.6b` rend 1 654 ms là où le point d'origine annonçait 2 098 ms sous 7,98. La cause était un pilote audio, `UA Mixer Engine`, qui confisquait un cœur sur dix depuis seize jours ; l'arrêter a retiré 22 à 36 % de latence à tous les paliers. | Une expérience faite pour reproduire le point manquant, qui a rendu un verdict opposé à celui qu'elle devait confirmer. |

9 of these 10 are now held by a named test, so the same mistake fails the build rather than reaching a reader.
<!-- /figures:retractations -->
